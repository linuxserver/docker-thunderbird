pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'THUNDERBIRD_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-thunderbird'
    CONTAINER_NAME = 'thunderbird'
    DOCKERHUB_IMAGE = 'linuxserver/thunderbird'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/thunderbird'
    PR_DOCKERHUB_IMAGE = 'lspipepr/thunderbird'
    DIST_IMAGE = 'ubuntu'
    MULTIARCH = 'true'
    CI = 'true'
    CI_WEB = 'true'
  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET http://ppa.launchpad.net/mozillateam/ppa/ubuntu/dists/noble/main/binary-amd64/Packages.gz | gunzip |grep -A 7 -m 1 'Package: thunderbird' | awk -F ': ' '/Version/{print $2;exit}' ''',
            returnStdout: true).trim()
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-thunderbird/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-thunderbird\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-thunderbird\" \
          ...
          --label \"org.opencontainers.image.description=[Thunderbird](https://www.thunderbird.net/) is a free and open-source personal information manager primarily used as an e-mail client with a calendar and contactbook, as well as an RSS feed reader, chat client, and news client.\" \
